...
def main():
    ...
    # step 2: predict
    test_users = pd.read_csv(os.path.join("/mp6_test_data", "{}_users.csv".format(test_set)))
    test_logs = pd.read_csv(os.path.join("/mp6_test_data", "{}_logs.csv".format(test_set)))
    y_pred = model.predict(test_users, test_logs)

    # step 3: grading based on accuracy
    y = pd.read_csv(os.path.join("/mp6_test_data", "{}_clicked.csv".format(test_set)))
