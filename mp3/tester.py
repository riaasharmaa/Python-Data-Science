...
def ind_test():
    ...
    location = scrape.reveal_secrets(
        browser(), f"http://localhost:{port}", expected_bfs_travellog
    )
    ind_points += 5
...
def main():
    ...
    if matches:
        result["errors"].append(
            f"You are not allowed to hardcode IP addresses like {matches[0][0]} in your code."
        )
        return result
